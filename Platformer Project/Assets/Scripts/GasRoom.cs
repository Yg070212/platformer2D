...
public class GasRoom : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            // Player가 알아야 할 필요성이 있겠죠. PlayerController <- 다른 클래스에서 나의 클래스를 어떻게 접근 할것인가?
            isGasState = true;
    ...
    private void Update() // PlayerController의 Update에서 작성하는 게 맞지만. 
    {
        if (isGasState)   // { } 가스 방에 들어갔을 때의 로직 작성한다, 체력을 깍이는 로직을 예시로 작성하였다.
        {
            ...
            //float checkTime = 2f;
            Timer += Time.deltaTime;  // 0.016. 컴퓨터 마다 다르다. 1Frame 생성하는 시간.
            if(Timer >= checkTime)
            {
                Timer = 0;
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.CompareTag("Player") && isStayOn)
        {
            Debug.Log($"플레이어가 가스 상태이므로 플레이어의 체력을 감소시키고 있다.");
            PlayerHP = PlayerHP - Damage;
            Debug.Log($"플레이어의 현재 체력 : {PlayerHP}");
        }
    }
